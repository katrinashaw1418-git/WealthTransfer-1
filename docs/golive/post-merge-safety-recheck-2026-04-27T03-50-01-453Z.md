...
* Started: 2026-04-27T03:50:01.453Z
* Finished: 2026-04-27T03:50:31.768Z
* Wall-clock duration: 30270ms (30.3s)
...
| lifecycle: reversal symmetry | PASS | pre wallet=0.00000000 ledger=0; mid wallet=250.00000000 ledger=250.00000000; post wallet=0.00000000 ledger=0.00000000; audit rows visible=2/2 (forward=1908, reversal=1909) |
...
_The recheck is RED but no FAIL/SKIP gates were parsed from the script output (exit=0, parsed gates=14, expected=14). The sub-script likely crashed before reaching the canonical reporter — see the raw stdout/stderr below._
...
[operator-alerts] suppressed duplicate for source=wallet-ledger-reconciliation (alertId=2606, occurrences=7)
[operator-alerts] suppressed duplicate for source=wallet-ledger-reconciliation (alertId=2605, occurrences=7)

PASS deposit idempotency — 3 attempts, 1 transaction (id=1871)
PASS deposit idempotency payload-hash guard — different payload with reused key would be rejected
PASS pending no ledger impact — tx#1872 has 0 ledger entries
PASS settlement single ledger entry — 2 entries on tx#1873; second post threw LedgerDoublePostError
PASS failure no ledger impact — tx#1874 has 0 ledger entries
...
PASS concurrent post race — tx#1877: 1 fulfilled, 1 LedgerDoublePostError, 2 entries
PASS reverseSettledDeduction unwinds ledger (status flip) — deduction#384 → reversed (reversalTx#1880)
PASS reverseSettledDeduction unwinds ledger (reversal tx row) — tx#1880 type=adviser_fee_deduction_reversal key=fee_deduction_384_reversal
...
PASS reverseSettledDeduction is idempotent — 2nd call returned same row, +0 transactions, +0 ledger entries; exactly 1 row with idempotency_key 'fee_deduction_384_reversal'
...
PASS 6. append-only notes preserved AND no PATCH/DELETE route exists — v1 unchanged after v2 post; v2.previousNoteId=392 -> v1.id=392; scanned 45 routes, 0 PATCH/DELETE touch notes
...
PASS 8. ack row captures the full eleven-confirm disclaimer — ack row id=190 has 11/11 confirm flags=true and signatureName='WPC Test Client'; viewer gate returned 200
PASS 9. non-adviser (admin/client) rejected on adviser CRUD AND admin retains audit-log read access — admin observed 68 adviser audit rows (read OK); admin→objective:403, admin→note:403, client→note:403; 0 leakage
...
PASS 11. review_pending blocks objective/document/transition writes; notes still allowed — objectives=423, documents=423, transition=423 (reason='record_locked_under_review'); note=200 (id=394); leakage Δ=0/0/0 (obj/doc/ver)
PASS 12. client-document upload+download round-trip via real object storage with cross-client gate — routes registered; bytes round-tripped (44B); storageKey='client-documents/37/0d8f04e7134eef661a027fe6026dd993.bin' computed by system; download route 200+exact bytes; cross-client probe 404 at both service and route layers
PASS 13. blocked-write audit row recorded by the gate for every gated child write — 5/5 probes (2 route + 3 service) threw 423 with reason='record_locked_under_review'; 5 audit rows written with action='advice_record.write_blocked', entityId='451', userId=39, before/after=null, adviceRecordStatus='review_pending'; per-action counts = {client_objective.create:2, client_document.create:2, client_document.upload:1}
...
MATCH_EPSILON = 0.01
Test user id: 45
[operator-alerts] suppressed duplicate for source=wallet-ledger-reconciliation (alertId=2608, occurrences=7)
[run1 summary] {
  pairsChecked: 4,
  matches: 3,
  mismatches: 1,
...
[wallet-ledger-reconciliation] operator alert suppressed (acknowledged) {
  userId: 45,
  currency: 'AUD',
  acknowledgementId: 142,
  acknowledgedAt: 2026-04-27T03:50:20.155Z,
...
[operator-alerts] suppressed duplicate for source=wallet-ledger-reconciliation (alertId=2609, occurrences=7)
[run3 summary] {
  pairsChecked: 4,
  matches: 3,
  mismatches: 1,
...
[wallet-ledger-reconciliation] operator alert suppressed (acknowledged) {
  userId: 45,
  currency: 'AUD',
  acknowledgementId: 143,
  acknowledgedAt: 2026-04-27T03:50:20.357Z,
  acknowledgedDriftAmount: '50.00000000',
...
[operator-alerts] suppressed duplicate for source=wallet-ledger-reconciliation (alertId=2610, occurrences=7)
[run5 summary] {
  pairsChecked: 4,
  matches: 3,
  mismatches: 1,
...
[fx-refresh] rates updated at 2026-04-27T03:50:22.723Z
...
PASS insufficient funds writes no transactions row — idempotencyKey=fee_deduction_389
PASS insufficient funds writes no ledger entries — idempotencyKey=fee_deduction_389
PASS sufficient funds settles cleanly — status=settled, settledTransactionId=1911
PASS sufficient funds inserts exactly one transactions row — idempotencyKey=fee_deduction_389
...
PASS lifecycle: reversal symmetry — pre wallet=0.00000000 ledger=0; mid wallet=250.00000000 ledger=250.00000000; post wallet=0.00000000 ledger=0.00000000; audit rows visible=2/2 (forward=1908, reversal=1909)
