...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    // Create demo investment products. The 14 canonical products live in
    // `shared/product-catalogue.ts` so MemStorage, the seed script
    // (`scripts/seed-missing-products.ts`), and the cleanup script
    // (`scripts/cleanup-product-shelf.ts`) all share one source of truth and
    // cannot drift apart. Field formatting conventions and the four
    // cleanliness rules they're enforced against (no tilde shorthand, no
    // "FDIC", canonical lowercase risk_profile, "p.a." suffix on numeric
    // IRR) are documented inside that module.
    //
    // The .map() below adds the columns MemStorage stores in addition to the
    // catalogue fields: a stable demo id (1-based index matching the
    // historical seed order so existing demoUserInvestments rows still
    // resolve), createdAt, and the schema defaults for annualReturn /
    // returnMethod / isPublished (filled in by the second .map() below).
    // `assertCatalogueIsClean()` is a defence-in-depth runtime gate so a
    // fresh dev boot also refuses to seed a non-compliant catalogue, even
    // before the CI vitest gets a chance to run.
    assertCatalogueIsClean();
    const demoInvestmentProducts: Omit<InvestmentProduct, 'annualReturn' | 'returnMethod' | 'isPublished'>[] = PRODUCT_CATALOGUE.map((p, i) => ({
      id: i + 1,
      name: p.name,
      category: p.category,
