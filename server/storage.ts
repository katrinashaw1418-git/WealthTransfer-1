...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    // Source-of-truth catalogue for the 14-product adviser shelf. Mirrors
    // scripts/seed-missing-products.ts exactly so dev (MemStorage), demo, and
    // seeded production environments all show the same cleaned-up shelf.
    ...
    //     drift to sentence-case strings ("High", "Very High") silently
    //     breaks the suitability rule and the risk filter.
    ...
    const demoInvestmentProducts: Omit<InvestmentProduct, 'annualReturn' | 'returnMethod' | 'isPublished'>[] = [
      ...
      },
      {
        ...
        investmentStrategy: "FCS-protected high-yield savings account offering competitive interest rates for idle funds. ADI-issued deposit, covered up to $250,000 per ADI under the Financial Claims Scheme. Features instant access, no minimum balance requirements, and automated daily interest accrual with monthly compounding.",
        targetNetIrr: "4.5–5.5% p.a.",
        grossIrr: null,
        moic: null,
        term: "Open-ended",
        structure: "ADI-issued savings deposit · FCS-protected (covered up to $250,000 per ADI under the Financial Claims Scheme)",
      ...
      },
      {
        ...
        investmentStrategy: "Premium ADI-issued deposit product backed by Australian government securities offering superior yields for larger balances. Features tiered interest rates and notice-based liquidity for sophisticated treasury management.",
        targetNetIrr: "2.5–3.5% p.a.",
        grossIrr: null,
        moic: null,
        term: "Open-ended",
        structure: "ADI-issued term deposit · FCS-protected (covered up to $250,000 per ADI under the Financial Claims Scheme) · 30-day notice",
        distributions: "Daily accrual",
  ...
  async createApplication(application: InsertApplication): Promise<Application> {
    ...
    const app: Application = { ...application, id, status: application.status || "email_unverified", entityName: application.entityName || null, abn: application.abn || null, consentOwnBehalf: application.consentOwnBehalf ?? false, consentAmlCtf: application.consentAmlCtf ?? false, consentContact: application.consentContact ?? false, consentGeneralAdvice: application.consentGeneralAdvice ?? false, reviewNote: null, emailVerified: application.emailVerified ?? false, emailOtp: application.emailOtp ?? null, emailOtpExpiry: application.emailOtpExpiry ?? null, emailOtpAttempts: application.emailOtpAttempts ?? 0, createdAt: new Date(), reviewedAt: null };
